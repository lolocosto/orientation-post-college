...
class OnisepAPI {
    constructor(token = null, appId = null) {
        ...
        this.datasets = {
            ...
            enseignements_optionnels_2nde: '5fa591127f501',  // Enseignements optionnels 2nde GT
            enseignements_specialite_1ere: '5fa591144e29a'   // Enseignements spécialité 1ère};
        }
        this.token = token;
    ...
    _sleep(ms) {
        ...
        /**
     * Récupère les enseignements optionnels de 2nde GT pour un établissement
     */
    async getEnseignementsOptionnels2nde(uai) {
        const url = new URL(`${this.baseURL}/dataset/${this.datasets.enseignements_optionnels_2nde}/search`);
        url.searchParams.append('q', uai);
        url.searchParams.append('size', 100);
        ...
        try {
            ...
            const response = await this._fetchWithRetry(
                () => fetch(url, { headers: this._getHeaders() }),
                `enseignements 2nde ${uai}`,
                3
            );
            
            if (!response.ok) {
                console.warn(`Erreur HTTP ${response.status} pour enseignements 2nde ${uai}`);
                return [];
            }
            
            const data = await response.json();
            
            // Filtrer pour cet UAI
            const filtered = (data.results || []).filter(ens => 
                ens.ens_code_uai === uai
            );
            
            console.log(`Enseignements optionnels 2nde ${uai}: ${filtered.length} trouvés`);
            
            return filtered;
        } catch (error) {
            console.error(`Erreur getEnseignementsOptionnels2nde pour ${uai}:`, error);
            return [];
        }
    }
    
    /**
     * Récupère les enseignements de spécialité de 1ère pour un établissement
     */
    async getEnseignementsSpecialite1ere(uai) {
        const url = new URL(`${this.baseURL}/dataset/${this.datasets.enseignements_specialite_1ere}/search`);
        url.searchParams.append('q', uai);
        url.searchParams.append('size', 100);
        ...
        try {
            ...
            const response = await this._fetchWithRetry(
                () => fetch(url, { headers: this._getHeaders() }),
                `enseignements 1ère ${uai}`,
                3
            );
            
            if (!response.ok) {
                console.warn(`Erreur HTTP ${response.status} pour enseignements 1ère ${uai}`);
                return [];
            }
            ...
            console.log(`Enseignements spécialité 1ère ${uai}: ${filtered.length} trouvés`);
            
            return filtered;
        } catch (error) {
            console.error(`Erreur getEnseignementsSpecialite1ere pour ${uai}:`, error);
            return [];
        }
    }
...
function createTables() {
    ...
    // Table des options de 2nde GT par établissement
    ...
    // Table des spécialités de 1ère G par établissement  
    ...
    try {
        ...
        document.getElementById('lycee-details-body').innerHTML = `
            <div style="margin-bottom: 25px; padding-bottom: 20px; border-bottom: 1px solid var(--border);">
                ...
                ${url_onisep ? `<p><strong>🔗 Fiche Onisep :</strong> <a href="${url_onisep}" target="_blank" style="color: var(--primary); text-decoration: underline;">Voir sur Onisep.fr</a></p>` : ''}
            </div>
...
async function updateDatabase(data) {
    try {
        ...
        for (const l of data.lycees) {
            // Vérifier si c'est un lycée GT (pas uniquement professionnel)
            ...
            if (estGT && l.code_uai) {
                try {
                    await extractAndStoreOptions2ndeGT(l.code_uai, db);
                    await extractAndStoreSpecialites1ereG(l.code_uai, db);
                    compteurGT++;
                    
                    if (compteurGT % 10 === 0) {
                        console.log(`   ${compteurGT} lycées GT traités...`);
                    }
                } catch (e) {
                    console.error(`Erreur extraction enseignements ${l.code_uai}:`, e);
                }
            }
        }
        
        console.log(`✅ Enseignements GT extraits pour ${compteurGT} lycées`);
...
async function showDiplomeDetails(intitule, lyceesCount) {
    ...
    try {
        ...
        let parcoursHtml = '';
        if (intitule && (intitule.includes('Bac pro') || intitule.includes('Bac Pro'))) {
            console.log(`[DEBUG DIPLOME] C'est un Bac Pro !`);
            try {
                const parcours = await getParcoursBackPro(intitule, db);
...
// ═════════════════════════════════════════════════════════════
// FONCTIONS POUR LES ENSEIGNEMENTS GT
// ═════════════════════════════════════════════════════════════

/**
 * Extrait et stocke les options de 2nde GT pour un établissement
 */
async function extractAndStoreOptions2ndeGT(uai, db) {
    if (!window.onisepAPI) return;
    
    try {
        const options = await window.onisepAPI.getEnseignementsOptionnels2nde(uai);
        ...
        for (const opt of options) {
            ...
            if (libelle && libelle.trim()) {
                ...
                const optionsList = libelle.split('\\/').map(o => o.trim()).filter(o => o);
        ...
        console.error(`Erreur extraction options 2nde GT ${uai}:`, error);
    }
}

/**
 * Extrait et stocke les spécialités de 1ère G pour un établissement
 */
async function extractAndStoreSpecialites1ereG(uai, db) {
    if (!window.onisepAPI) return;
    
    try {
        const specialites = await window.onisepAPI.getEnseignementsSpecialite1ere(uai);
        
        for (const spec of specialites) {
            // Le champ peut contenir plusieurs spécialités séparées par \/ 
            const libelle = spec.ens_lib_voe_ins || spec.enseignement || '';
        ...
        console.error(`Erreur extraction spécialités 1ère G ${uai}:`, error);
    }
}
/**
 * Stocke les parcours Bac Pro depuis les données statiques
 */
...
/**
 * Récupère les options de 2nde GT pour un établissement
 */
async function getOptions2ndeGT(uai, db) {
    try {
        ...
        return rows.map(r => r.option_libelle);
    } catch (error) {
...
/**
 * Récupère les spécialités de 1ère G pour un établissement
 */
async function getSpecialites1ereG(uai, db) {
    try {
        ...
        return rows.map(r => r.specialite_libelle);
    } catch (error) {
...
/**
 * Récupère le parcours Bac Pro pour un diplôme
 */
async function getParcoursBackPro(diplomeLibelle, db) {
    try {
        const row = await db.get(
            'SELECT * FROM parcours_bac_pro WHERE diplome LIKE ?',
            [`%${diplomeLibelle}%`]
        );
        return row || null;
    } catch (error) {
...
/**
 * Génère le HTML pour afficher les enseignements GT dans une fiche établissement
 */
...
/**
 * Génère le HTML pour afficher le parcours Bac Pro dans une fiche diplôme
 */
function generateParcoursProHtml(parcours) {
    if (!parcours) return '';
    
    return `
    <div class="diplome-parcours" style="margin-top: 15px; padding: 15px; background: #fff3cd; border-left: 4px solid #ffc107; border-radius: 4px;">
        <h5 style="margin-top: 0; margin-bottom: 10px; color: #856404;">📌 Parcours professionnel</h5>
        <div style="font-size: 13px;">
            <strong>Famille de métiers :</strong> ${parcours.famille}<br>
