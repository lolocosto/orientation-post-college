...
function showGuideBacPro() {
    if (!window.FAMILLES_METIERS_BAC_PRO) {
        alert('❌ Données non chargées');
        return;
    }
    
    let html = '<div style="max-width: 800px; margin: 0 auto; padding: 20px;">';
    html += '<h2>🎓 Parcours Bac Professionnel - Familles de métiers</h2>';
    html += '<p style="margin-bottom: 20px;">En 2nde professionnelle, l\'élève entre dans une <strong>famille de métiers</strong> qui regroupe plusieurs spécialités proches. Le choix du Bac Pro précis se fait en fin de 2nde ou début de 1ère.</p>';
    
    for (const [famille, data] of Object.entries(window.FAMILLES_METIERS_BAC_PRO)) {
        html += `<div style="border: 1px solid #ddd; padding: 15px; margin-bottom: 15px; border-radius: 8px; background: #f9f9f9;">`;
        html += `<h3 style="color: var(--primary); margin-top: 0;">${famille}</h3>`;
        html += `<p style="font-style: italic; color: #666;">📌 ${data['2nde_commune']}</p>`;
        html += `<p style="margin: 10px 0 5px 0; font-weight: 600;">Bacs professionnels accessibles :</p>`;
        html += '<ul style="margin: 0; padding-left: 20px;">';
        data.bacs_pro.forEach(bac => {
            html += `<li>${bac}</li>`;
        });
        html += '</ul></div>';
    }
    
    html += '<p style="margin-top: 20px; font-size: 13px; color: #666;">Source : <a href="https://www.onisep.fr/formation/apres-la-3-la-voie-professionnelle/les-diplomes-de-la-voie-pro/le-bac-professionnel/les-familles-de-metiers" target="_blank">Onisep</a></p>';
    html += '</div>';
    
    showModalGuide('Guide Bac Professionnel', html);
}

function showGuide2ndeGT() {
    if (!window.ENSEIGNEMENTS_OPTIONNELS_2NDE_GT) {
        alert('❌ Données non chargées');
        return;
    }
    
    const data = window.ENSEIGNEMENTS_OPTIONNELS_2NDE_GT;
    
    let html = '<div style="max-width: 800px; margin: 0 auto; padding: 20px;">';
    html += '<h2>📖 Enseignements optionnels en 2nde Générale et Technologique</h2>';
    html += '<p style="margin-bottom: 20px;">En plus des enseignements communs, l\'élève peut choisir des enseignements optionnels pour découvrir de nouveaux domaines.</p>';
    
    // Options générales
    html += `<div style="border: 2px solid var(--primary); padding: 15px; margin-bottom: 20px; border-radius: 8px;">`;
    html += `<h3 style="color: var(--primary); margin-top: 0;">${data.optionnels_generaux.titre}</h3>`;
    data.optionnels_generaux.liste.forEach(opt => {
        html += `<div style="margin-bottom: 15px; padding: 10px; background: #f9f9f9; border-radius: 5px;">`;
        html += `<strong>${opt.nom}</strong> - ${opt.volume}<br>`;
        if (opt.options) {
            html += `<em>Options : ${opt.options.join(', ')}</em><br>`;
        }
        html += `<span style="color: #666; font-size: 13px;">${opt.description}</span>`;
        html += `</div>`;
    });
    html += '</div>';
    
    // Options technologiques
    html += `<div style="border: 2px solid #2196F3; padding: 15px; margin-bottom: 20px; border-radius: 8px;">`;
    html += `<h3 style="color: #2196F3; margin-top: 0;">${data.optionnels_technologiques.titre}</h3>`;
    data.optionnels_technologiques.liste.forEach(opt => {
        html += `<div style="margin-bottom: 15px; padding: 10px; background: #f9f9f9; border-radius: 5px;">`;
        html += `<strong>${opt.nom}</strong> - ${opt.volume}<br>`;
        html += `<span style="color: #666; font-size: 13px;">${opt.description}</span>`;
        html += `</div>`;
    ...
    html += `<div style="background: #fff3cd; padding: 15px; border-radius: 8px; border: 1px solid #ffc107;">`;
    html += `<strong>⚠️ Règles :</strong><br>`;
    html += `• ${data.regles.cumul}<br>`;
    html += `• ${data.regles.latin_grec}<br>`;
    html += `• ${data.regles.note}`;
    html += `</div>`;
    
    html += '<p style="margin-top: 20px; font-size: 13px; color: #666;">Source : <a href="https://www.onisep.fr/orientation/le-lycee/les-enseignements-optionnels-en-seconde" target="_blank">Onisep</a></p>';
    html += '</div>';
    
    showModalGuide('Guide 2nde GT', html);
}

function showGuide1ereTermGT() {
    if (!window.ENSEIGNEMENTS_SPECIALITE_VOIE_GENERALE) {
        alert('❌ Données non chargées');
        return;
    }
    
    const data = window.ENSEIGNEMENTS_SPECIALITE_VOIE_GENERALE;
    
    let html = '<div style="max-width: 900px; margin: 0 auto; padding: 20px;">';
    html += '<h2>🎯 Enseignements de spécialité - Voie Générale</h2>';
    
    // Première
    html += '<div style="border: 2px solid var(--primary); padding: 20px; margin-bottom: 20px; border-radius: 8px;">';
    html += '<h3 style="color: var(--primary); margin-top: 0;">📘 En Première</h3>';
    html += '<h4>Tronc commun (pour tous) :</h4><ul>';
    data.premiere.tronc_commun.forEach(item => html += `<li>${item}</li>`);
    html += '</ul>';
    
    html += `<h4>Enseignements de spécialité : ${data.premiere.specialites.nombre}</h4>`;
    html += `<p><em>Volume : ${data.premiere.specialites.volume}</em></p><ul>`;
    data.premiere.specialites.liste.forEach(item => {
        if (typeof item === 'string') {
            html += `<li>${item}</li>`;
        } else {
            html += `<li><strong>${item.nom}</strong>`;
            if (item.options) html += ` : ${item.options.join(', ')}`;
    ...
    // Terminale
    html += '<div style="border: 2px solid #4CAF50; padding: 20px; margin-bottom: 20px; border-radius: 8px;">';
    html += '<h3 style="color: #4CAF50; margin-top: 0;">📗 En Terminale</h3>';
    html += '<h4>Tronc commun (pour tous) :</h4><ul>';
    data.terminale.tronc_commun.forEach(item => html += `<li>${item}</li>`);
    html += '</ul>';
    
    html += `<h4>Enseignements de spécialité : ${data.terminale.specialites.nombre}</h4>`;
    html += `<p><em>Volume : ${data.terminale.specialites.volume}</em></p>`;
    html += `<p style="background: #fff3cd; padding: 10px; border-radius: 5px;">⚠️ ${data.terminale.specialites.note}</p>`;
    html += '</div>';
    
    html += '<p style="margin-top: 20px; font-size: 13px; color: #666;">Source : <a href="https://www.onisep.fr/formation/apres-la-3-la-voie-generale-et-technologique" target="_blank">Onisep</a></p>';
    html += '</div>';
    
    showModalGuide('Guide 1ère/Term GT', html);
}

function showGuideBacTechno() {
    if (!window.SERIES_BAC_TECHNO) {
        alert('❌ Données non chargées');
        return;
    }
    
    const data = window.SERIES_BAC_TECHNO;
    
    let html = '<div style="max-width: 800px; margin: 0 auto; padding: 20px;">';
    html += `<h2>⚙️ ${data.titre}</h2>`;
    html += '<p style="margin-bottom: 20px;">8 séries de baccalauréat technologique, chacune avec ses spécificités.</p>';
    
    data.series.forEach(serie => {
        html += `<div style="border: 1px solid #ddd; padding: 15px; margin-bottom: 15px; border-radius: 8px; background: #f9f9f9;">`;
        html += `<h3 style="color: var(--primary); margin-top: 0;">${serie.sigle} - ${serie.nom}</h3>`;
        
        if (serie.specialites_premiere) {
            html += '<p style="margin: 5px 0;"><strong>En 1ère :</strong> ' + serie.specialites_premiere.join(', ') + '</p>';
        }
        
        if (serie.specialites_terminale) {
            html += '<p style="margin: 5px 0;"><strong>En Terminale (au choix) :</strong></p><ul style="margin: 5px 0; padding-left: 20px;">';
            serie.specialites_terminale.forEach(spec => html += `<li>${spec}</li>`);
            html += '</ul>';
        }
        
        if (serie.specialites) {
            html += '<p style="margin: 5px 0;"><strong>Spécialités :</strong></p><ul style="margin: 5px 0; padding-left: 20px;">';
            serie.specialites.forEach(spec => html += `<li>${spec}</li>`);
            html += '</ul>';
        }
        
        if (serie.options) {
            html += '<p style="margin: 5px 0;"><strong>Options :</strong> ' + serie.options.join(', ') + '</p>';
        }
        
        if (serie.note) {
            html += `<p style="margin: 5px 0; font-style: italic; color: #666;">${serie.note}</p>`;
        }
        
        html += '</div>';
    });
    
    html += '<p style="margin-top: 20px; font-size: 13px; color: #666;">Source : <a href="https://www.onisep.fr" target="_blank">Onisep</a></p>';
    html += '</div>';
    
    showModalGuide('Guide Bac Techno', html);
}

function showModalGuide(title, content) {
    ...
    modalContent.style.cssText = 'background: white; border-radius: 12px; max-width: 95%; max-height: 90%; overflow-y: auto; box-shadow: 0 10px 40px rgba(0,0,0,0.3);';
    
    const modalHeader = document.createElement('div');
    modalHeader.style.cssText = 'padding: 20px; border-bottom: 1px solid #ddd; display: flex; justify-content: space-between; align-items: center; position: sticky; top: 0; background: white; z-index: 1;';
    modalHeader.innerHTML = `<h2 style="margin: 0;">${title}</h2><button onclick="this.closest('div[style*=fixed]').remove()" style="border: none; background: none; font-size: 24px; cursor: pointer; padding: 5px 10px;">&times;</button>`;
